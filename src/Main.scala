...
//      println(s" value = $value")
    ...
    var sorted :Array[Double] = Array()
    if(evolution.isMax) {
      sorted = result.slice(0, index).sortWith(_ > _)
    }
    else {
      sorted = result.slice(0, index).sortWith(_ < _)
    }
    ...
    println(s"func: 3-sin($j*x1)^2-cos($j*x2)^2 (x1,x2 in" +
      s"[$from,$to]) ${if(evolution.isMax) "max" else "min"} = ${sorted(0)}")
